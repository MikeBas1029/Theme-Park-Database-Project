...
class CustomerNotifications(SQLModel, table=True):
    ...
    notification_id: str = Field(
        default_factory=lambda: CustomerNotifications.generate_random_id(),
    ...
    customer_id: str = Field(
        sa_column=Column(
            ...
            nullable=False,
            comment="Foreign Key to customer_id in customers table."
        )
    )
    ...
    message: str = Field(
        sa_column=Column(
            mysql.VARCHAR(150), 
            nullable=False, 
            comment="Notification message to the customer"
        )
    )
    ...
    customer: "Customers" = Relationship(back_populates="cust_notification")
