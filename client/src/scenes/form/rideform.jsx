...
const userSchema = yup.object().shape({
    ...
    height_requirement: yup.number().required("Height requirement is required").positive("Height must be a positive number"),
    capacity: yup.number().required("Capacity is required").positive("Capacity must be a positive number"),
    status: yup.string().oneOf(["OPEN", "CLOSED"], "Status must be OPEN or CLOSED"), // Adjust based on your needs
});
...
const RideForm = () => {
    ...
    const handleFormSubmit = async (values) => {
        ...
        try {
            const response = await axios.post('https://theme-park-backend.ambitioussea-02dd25ab.eastus.azurecontainerapps.io/api/v1/rides/', requestBody);
    ...
    <Box  m="20px">
        ...
        <Header title="ADD A NEW RIDE" subtitle="ADD A NEW RIDE TO THE LIST OF PARK RIDES" />
        <Formik 
