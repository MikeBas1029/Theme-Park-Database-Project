...
const VisitForm = ({ isOpen }) => {
	...
	const handleFormSubmit = async (values) => {
		...
		const requestBody = {
			...
			start_date:
				values.start_date || new Date().toISOString().split("T")[0], // Default to today if not provided
			employee_type: values.employee_type,
			hourly_wage:
				values.employee_type === "Hourly"
					? parseFloat(
							values.hourly_wage.replace(/[^0-9.-]+/g, "")
						) || 0
					: 0, // Ensure it's a number
			salary:
				values.employee_type === "Salary"
					? parseFloat(values.salary.replace(/[^0-9.-]+/g, "")) || 0
					: 0, // Ensure it's a number
		...
		try {
			const response = await axios.post(
				"https://theme-park-backend.ambitioussea-02dd25ab.eastus.azurecontainerapps.io/api/v1/visits/",
				requestBody
			);
	...
	return (
		...
		>
			<IconButton onClick={() => navigate("/customervisits")}>
				<ArrowBackIcon sx={{ fontSize: "30px", color: "grey" }} />
			</IconButton>

			<Header
				title="LEAVE NEW VISIT FEEDBACK"
				subtitle="Will be moved to customer side, on employee side temporarily"
			/>
			<Formik
