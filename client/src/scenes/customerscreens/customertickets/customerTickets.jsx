		...
		const fetchTickets = async () => {
			...
			try {
				const response = await fetch(
					`https://theme-park-backend.ambitioussea-02dd25ab.eastus.azurecontainerapps.io/api/v1/tickets/user/${customer_id}`
				);
					...
					{selectedTicket && (
						<>
							<Typography variant="h5">
								{" "}
								Ticket: {selectedTicket.ticket_type}
							</Typography>
							<Typography variant="body1">
								{" "}
								Purchased on: {selectedTicket.purchase_date}
							</Typography>
							<Typography variant="h6">
								Status: {selectedTicket.status}
							</Typography>
							<Typography variant="subtitle1">
								Activated: {selectedTicket.start_date}
							</Typography>
